...
// func drawGraph(trip *Trip, pts []Node, fn string) {
// 	cities := makePlotter(pts)
...
// func makePlotterB(nn []Node) plotter.XYs {
// 	var xys plotter.XYs
// 	for _, v := range nn {
...
// func plotDataB(cities, path plotter.XYs, fn string) error {
// 	filename := fn + ".png"
...
func writeGraphToFile(gg *[]Generation) {
	...
	fmt.Fprintf(file, "Generation,Best,Average,Worst\n")

	for _, v := range *gg {
		fmt.Fprintf(file, "%v,%v,%v,%v\n", v.generation, v.bestCost, v.averageCost, v.worstCost)
	}
}
